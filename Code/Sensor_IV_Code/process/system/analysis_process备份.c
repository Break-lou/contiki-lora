...
void calibrate_next_sample_start_line_by_mils(uint64_t timestamp)
{
	...
	#if DEBUG_PRINT
		printf("sample ellapse = %lld \n", ellapse);
	#endif
	remaining = ellapse % ((uint64_t)node_parameter.SSTI*1000);			//剩余的mils
	#if DEBUG_PRINT
		printf("sample remaining = %lld \n", remaining);
	#endif
	remaining = node_parameter.SSTI*1000 - remaining;	
	if(remaining>50 && remaining<node_parameter.SSTI*2000)
	{
		#if DEBUG_PRINT
			printf("sample remaining mils = %lld \n", remaining);
		#endif
